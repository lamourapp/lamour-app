...
- ⏸ **п.6** — `setTimeout(400/800)` у CreateEntryModal **залишено**. Прямий
  фікс не такий простий: pricing.ts ще читає 4 lookup-поля
  (`salonPctForServiceLookup`, `masterPctForMaterialsLookup`,
  `masterCompensationTypeLookup`, `hoursLookup`) які Airtable заповнює
  асинхронно після створення. Без затримки нові записи короткий час
  показуватимуть нулі. **Правильний шлях** — додати fixed*-снепшоти цих
  полів на POST і навчити pricing.ts читати з fixed коли є. Окрема задача.
